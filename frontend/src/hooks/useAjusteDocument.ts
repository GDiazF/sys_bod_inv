...
import { cloneAjuste, MOCK_AJUSTE, type AjusteDocument } from '@/mocks/documents/ajuste'
import type { DocumentStatus } from '@/mocks/documents/types'

const MOCK_DELAY_MS = 550
const ACTION_DELAY_MS = 900

export type UseAjusteDocumentResult = {
  status: DataViewStatus
  document: AjusteDocument | null
  setDocument: React.Dispatch<React.SetStateAction<AjusteDocument | null>>
  refetch: () => void
...
export function useAjusteDocument(): UseAjusteDocumentResult {
  const [status, setStatus] = useState<DataViewStatus>('loading')
  const [document, setDocument] = useState<AjusteDocument | null>(null)
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setDocument(cloneAjuste(MOCK_AJUSTE))
      setStatus('success')
    }, MOCK_DELAY_MS)
